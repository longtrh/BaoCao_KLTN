...
public class FrmChonThuoc extends JFrame implements ActionListener{
	...
	private LapDonDatDAO lapDonDatDAO = new LapDonDatDAO();
	    ...
	    while (check) {
	        try {
	            ...
	            if (soLuongThuocObj == null) {
	                check = false; // Thoát khỏi vòng lặp nếu nhấn Cancel
	            } else if (soLuongThuocObj.matches("^[0-9]+$")) { // Kiểm tra nhập số
	                int soLuongThuocInt = Integer.parseInt(soLuongThuocObj);
	                
	                // Kiểm tra số lượng không vượt quá 5
	                if (soLuongThuocInt > 5) {
	                    JOptionPane.showMessageDialog(null, "Số lượng thuốc không được vượt quá 5!");
	                ...
	                if ((lapDonDatDAO.soLuongThuocDaBan(maThuoc) + soLuongThuocInt) <= lapDonDatDAO.soLuongNhap(maThuoc)) {
	                    FrmLapDonDat.txtSoLuong.setText(soLuongThuocObj);
	                    FrmLapDonDat.danhSachThuocTheoMaThuoc(maThuoc);
	                    this.setVisible(false);
	                    check = false;
	                }  else {
	                    JOptionPane.showMessageDialog(this,
	                        "Số Lượng Thuốc Cần Mua Vượt Giới Hạn Thuốc Trong Kho!\nNhập Số Lượng Ít Hơn " +
	                        (lapHoaDonDAO.soLuongNhap(maThuoc) - lapHoaDonDAO.soLuongThuocDaBan(maThuoc)));
	                }
	            } else {
