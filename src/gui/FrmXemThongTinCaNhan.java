	...
	public void actionPerformed(ActionEvent e) {
		...
		if (obj.equals(btnCapNhat)) {
		    ...
		    if (matKhauMoi != null && !matKhauMoi.trim().isEmpty()) {
		        ...
		        // Giữ mật khẩu cũ nếu không có mật khẩu mới
		        taiKhoan.setMatKhau(thongTinCaNhanDAO.getMatKhauHienTai(nhanVien.getMaNhanVien()));
		    }
		    ...
		    boolean isDuplicate = thongTinCaNhanDAO.kiemTraTrungThongTinCaNhan(
		    ...
		    if (isDuplicate) {
		        JOptionPane.showMessageDialog(this,
		                "Số điện thoại hoặc CMND đã tồn tại trong hệ thống!",
		                "Cảnh báo",
		    ...
		    Object check = JOptionPane.showConfirmDialog(this, "Chắc chắn sửa thông tin?");
		    if (check.equals(0)) {
		        if (kiemTraDuLieu()) {
		            if (!txtTenDangNhap.getText().equalsIgnoreCase("ADMIN")) {
