...
//		Matcher tenTaiKhoanMatcher = tenTaiKhoanPatern.matcher(txtTenTaiKhoan.getText());
		try {
			if (txtTenTaiKhoan.getText().trim().equalsIgnoreCase("")) {
				...
				return false;
			} else if (!txtTenTaiKhoan.getText().trim().matches("[A-Za-z0-9]{3,}")) {
				JOptionPane.showMessageDialog(this, "Tên tài khoản có 3 ký tự trở lên, bao gồm số và chữ !");
				txtTenTaiKhoan.selectAll();
				txtTenTaiKhoan.requestFocus();
				return false;
			}
...
//			return false;
	...
	public boolean kiemTraUserNameTonTai() {
	    try {
	        // Lấy danh sách tên tài khoản từ database
	        List<String> listTenTaiKhoan = nvDao.getAllTenTaiKhoan();

	        // Kiểm tra nếu danh sách null hoặc rỗng
	        if (listTenTaiKhoan == null || listTenTaiKhoan.isEmpty()) {
	            return true; // Không có tài khoản nào, tài khoản có thể thêm
	        }
	        ...
	        for (String tenTk : listTenTaiKhoan) {
	            if (tenTk.equalsIgnoreCase(tenTaiKhoan)) { // So sánh không phân biệt chữ hoa/thường
	        ...
	        e.printStackTrace();
	        JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi khi kiểm tra tên tài khoản!", "Lỗi", JOptionPane.ERROR_MESSAGE);
	        return false; // Lỗi xảy ra khi kiểm tra
	    }

	    return true; // Tài khoản không tồn tại, có thể thêm
	}
