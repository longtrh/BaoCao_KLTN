...
public class FrmLapDonDat extends JFrame implements ActionListener,MouseListener,TableModelListener,KeyListener{
	...
	private static LapDonDatDAO lapHoaDonDAO = new LapDonDatDAO();
	/**
	...
	public FrmLapDonDat() {
		...
		lblLHD.setBounds(523,11 , 353, 38);
		...
		TitledBorder southTitleBorder2=new TitledBorder(southborder2,"Thông tin đơn đặt");
		...
		btnThemHoaDon = new JButton("Lập đơn đặt mới");
		...
		btnThemHoaDon.setBounds(768, 93, 181, 46);
...
//		btnThemDiaChi.setBounds(591, 116, 36, 23);
		...
		btnThoat.setBounds(1012, 94, 129, 45);
		jp2.add(btnThoat);
...
//		jp1.add(btnThemDiaChi);
	...
	public boolean addHoaDon() {
		...
		int n =0;
		try {
			stmt = con.prepareStatement("insert into dbo.DonDatThuoc(MaNhanVien,TongTien) values\r\n" + 
					"(?,?)");
	...
	public void themHoaDonVaChiTietHoaDonKeDon() {
		themHoaDonKeDon();
		CT_DonDatThuoc chiTietHoaDon = new CT_DonDatThuoc();
		ArrayList<CT_DonDatThuoc> listCT_HD = new ArrayList<CT_DonDatThuoc>();
		List data = tablemodel.getDataVector();
		Object[] list1 = data.toArray();
		...
		for(int k=0;k<list1.length;k++) {
			...
			try {
				chiTietHoaDon = new CT_DonDatThuoc(maHoaDon,Integer.parseInt(tablemodel.getValueAt(k,1).toString()),Float.parseFloat(tablemodel.getValueAt(k, 6).toString()),Float.parseFloat(tablemodel.getValueAt(k, 8).toString()),Integer.parseInt(tablemodel.getValueAt(k, 7).toString()),tablemodel.getValueAt(k, 4).toString());
...
//				});
		...
		for(CT_DonDatThuoc hoaDon: listCT_HD) {
			lapHoaDonDAO.addCT_DonDat(hoaDon.getMaDonDat(), hoaDon.getMaThuoc(), hoaDon.getDonGia(), hoaDon.getGiamGia(), hoaDon.getSoLuong(), hoaDon.getDonViTinh());
		}
...
//		frmXuatHD.printingHoaDon();
	    ...
	    JOptionPane.showMessageDialog(null, "Đơn đặt kê đơn và chi tiết đã được thêm thành công!");
	}
	public void themHoaDonVaChiTietHoaDonKhongKeDon() {
		...
		Object[] list1 = data.toArray();
		String maHoaDon =lapHoaDonDAO.maHoaDon(lapHoaDonDAO.maNhanVien(FrmDangNhap.taiKhoan.getTenTaiKhoan()));
		System.out.println("Mã đơn đặt trong hàm themHoaDonVaChiTiet..: " + maHoaDon);
...
//		frmXuatHD.printingHoaDon();
	    tablemodel.getDataVector().removeAllElements();
	    tablemodel.fireTableDataChanged();
	    JOptionPane.showMessageDialog(null, "Đơn đặt và chi tiết đơn đặt đã được thêm thành công!");
	}
	@Override
