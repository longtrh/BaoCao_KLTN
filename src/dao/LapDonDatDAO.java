...
public class LapDonDatDAO {
	...
	public boolean addHoaDonLoaiKeDon(int maNhanVien,int maKhachHang,float tongTien) {
		...
		int n =0;
		try {
			stmt = con.prepareStatement("insert into dbo.DonDatThuoc(MaNhanVien,MaKhachHang,TongTien) values\r\n" + 
					"(?,?,?)");
	...
	public String maHoaDon(String maNhanVien) {
	    ...
	    try {
	        Connection con = ConectDatabase.getInstance().getConnection();
	        String sql = "SELECT TOP 1 MaDonDat FROM DonDatThuoc WHERE MaNhanVien = ? ORDER BY NgayLap DESC";
	        PreparedStatement stmt = con.prepareStatement(sql);
	        stmt.setString(1, maNhanVien);
	        ResultSet rs = stmt.executeQuery();
	        if (rs.next()) {
	            maHoaDon = rs.getString("MaDonDat"); // Lấy giá trị MaDonDat
	        }
	        System.out.println("Mã đơn đặt đã lấy được: " + maHoaDon);
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
	    return maHoaDon;
	...
	public boolean addCT_DonDat(String maHoaDon,int maThuoc,float donGia,float giamGia,int soLuong,String donViTinh) {
		Connection con = ConectDatabase.getInstance().getConnection();
		PreparedStatement stmt = null;
		int n =0;
		try {
			stmt = con.prepareStatement("insert into dbo.CT_DonDatThuoc values\r\n" + 
					"(?,?,?,?,?,?)");
			...
			// TODO: handle exception
			System.out.println("Mã đơn đặt: " + maHoaDon);
			System.err.println("Lỗi khi thêm chi tiết đơn đặt: " + e3.getMessage());
		}
		return n>0;
	}
